...
const data = {
    ...
    main: "app.js",
    scripts: {
        "start": "echo \"Error: use homey-start on terminal\" && exit 1",
        "test": "athom app run",
        "install": "athom app install",
        "build": "athom app build && npm run init:app",
        "vbuild": "athom app version",
        "postvbuild": "npm run init:app && npm run _postvbuild && git push origin master",
        "_postvbuild": "git commit -am v$npm_package_version",
        "deploy": "npm run release && athom app publish",
        "deploy:patch": "npm run vbuild -- patch && npm run deploy",
        "deploy:minor": "npm run vbuild -- minor && npm run deploy",
        "deploy:major": "npm run vbuild -- major && npm run deploy",
        "release": "git checkout master && git pull && git tag v$npm_package_version && git push origin v$npm_package_version",
        "init:app": "npm init --quiet -y 1>/dev/null",
        "createRepo": "hub create -d \"$npm_package_description\" -h $npm_package_homepage $npm_package_name && npm run init:app"
    },
